export interface EndpointSpecMap {}

export type EndpointKey = keyof EndpointSpecMap;
export type EndpointSpec<K extends EndpointKey> = EndpointSpecMap[K];
export type EndpointResponse<K extends EndpointKey> = EndpointSpec<K>["response"];
export type EndpointBody<K extends EndpointKey> = EndpointSpec<K>["body"];
export type EndpointQuery<K extends EndpointKey> = EndpointSpec<K>["query"];
export type EndpointPath<K extends EndpointKey> = EndpointSpec<K>["path"];
...
type ShorthandNumericEndpoint = `${string}.${number}`;
type ShorthandOptions = Omit<ApiHelperOptions, "body"> & { query?: Record<string, unknown> };
...
function endpointToPathAllSegments(endpoint: string): string {
...
function hasKnownMethodSuffix(endpoint: string): boolean {
  const lastDot = endpoint.lastIndexOf(".");
  if (lastDot < 0) return false;
  const suffix = endpoint.slice(lastDot + 1);
  return ["get", "post", "put", "patch", "delete", "head", "options"].includes(suffix);
}

function isNumericSuffix(endpoint: string): boolean {
  const lastDot = endpoint.lastIndexOf(".");
  if (lastDot < 0) return false;
  const suffix = endpoint.slice(lastDot + 1);
 ...
 */
export function api(
  endpoint: ShorthandNumericEndpoint,
...
export async function api<K extends EndpointKey>(
  ...
  const shorthandNumeric = isNumericSuffix(ep) && !hasKnownMethodSuffix(ep);
  const method = shorthandNumeric ? "GET" : endpointToMethod(ep);
  ...
  const path = shorthandNumeric ? endpointToPathAllSegments(ep) : endpointToPath(ep, params);
