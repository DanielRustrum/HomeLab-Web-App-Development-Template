...
def _find_src_root(start: Path) -> Path:
    for parent in [start] + list(start.parents):
        if (parent / "endpoint").exists() or (parent / "pages").exists():
            return parent
        if parent.name == "src":
            return parent
...
_DEFAULT_PAGES_DIR = _SRC_ROOT / "pages"
    ...
    *,
    ...
    if api_dir is None:
        ...
        if not api_dir_path.is_absolute():
            api_dir_path = _SRC_ROOT / api_dir_path
    ...
    if pages_dir is None:
        ...
        if not pages_dir_path.is_absolute():
            pages_dir_path = _SRC_ROOT / pages_dir_path

    if run_init:
        _run_init_file(init_path)

    router = ApiRouter(api_dir=api_dir_path, pages_dir=pages_dir_path)
...
class ApiRouter:
    ...
    def __init__(self, *, api_dir: Path, pages_dir: Path) -> None:
    ...
    def default(self, *vpath, **_params):
        ...
        if (
            method == "get"
            and self._tsx_route_exists(match["file"])
            and callable(getattr(endpoint_cls, "get", None))
        ):
